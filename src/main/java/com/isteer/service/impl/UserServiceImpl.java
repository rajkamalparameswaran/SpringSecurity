...
public class UserServiceImpl implements UserService {
	...
	public List<String> getErrorList(User user) {
		...
		if ((user.getUserName() == null) || (user.getUserFullName() == null) || (user.getUserEmail() == null)
			...
			try {
				if(userDao.toCheckDuplicateUserName(user.getUserName(),user.getUserId())!=0 || userDao.toCheckDuplicateUserEmail(user.getUserEmail(),user.getUserId())!=0)
				{
					exceptions.add(failedMsg.getNameOrEmailAlreadyExist());
				}
			} catch (SQLException e) {
				throw new SqlQueryException(StatusCode.SQLEXCEPTIONCODE.getCode(), failedMsg.getInvalidSqlQuery(), Arrays.asList(e.getLocalizedMessage()));
			}
		}
		return exceptions;
	}
	...
	public void userIdFoundAndExceptionThrower(Integer userId) {
		...
		if (idFound) {
			...
			Log4j2.getAuditlog().info(id+" "+userId+failedMsg.getUserIdNotFound());
	...
	public UserResponse addUser(User user) {
		...
		if (exceptions.isEmpty()) {
			try {
				...
				throw new SqlQueryException(StatusCode.ACCOUNTCREATEDFAILED.getCode(), failedMsg.getAccountCreationFailed(),
						exceptions);
			}
		}
		else {
			Log4j2.getAuditlog().info(failedMsg.getNotValidData());
			throw new SqlQueryException(StatusCode.ACCOUNTCREATEDFAILED.getCode(), failedMsg.getNotValidData(), exceptions);
		}
	}
	...
	public AlternativeReturnUser updateUser(User user) {
		...
		if (exceptions.isEmpty()) {
			try {
				...
				Log4j2.getAuditlog().info(id+" "+user.getUserId()+" "+successMsg.getAccountUpdated(), user.getUserId());
	...
	public String grantPermission(User user) {
		...
		try {
			...
			throw new SqlQueryException(StatusCode.ACCOUNTCREATEDFAILED.getCode(), failedMsg.getProcessFailed(), Arrays.asList(e.getMessage()));
		}
	}
	...
	public AddressesResponse getAddressByUserId(Integer userId) {
		...
		try {
			...
			throw new SqlQueryException(StatusCode.ACCOUNTFETCHINGFAILED.getCode(), failedMsg.getDataFetchProcssFailed(),
					exception);
		}
	}
	...
	public EndPointResponse addNewEndPoint(EndPoint endPoint) {
		...
		if (exception.isEmpty()) {
			try {
				...
				Log4j2.getAuditlog().error(e.getLocalizedMessage());
				throw new SqlQueryException(StatusCode.ENDPOINTACESSUPDATEDFAILED.getCode(), failedMsg.getEndPointCannotAdded(),Arrays.asList(e.getMessage()));
			}
		} else {
			Log4j2.getAuditlog().info(failedMsg.getNotValidData());
	...
	public EndPointResponse updateEndPointAccess(EndPoint endPoint) {
		...
		if (endPointName == null) {
			exception.add(failedMsg.getUserIdNotFound());
			Log4j2.getAuditlog().info(id+" "+endPoint.getEndPointId()+" "+ failedMsg.getUserIdNotFound(),endPoint.getEndPointId());
			throw new UserIdNotFoundException(StatusCode.USERIDNOTFOUND.getCode(), failedMsg.getProcessFailed(), exception);
		}
		if (endPoint.getAuthorities() == null) {
			exception.add(failedMsg.getNotValidData());
		}
		if (!exception.isEmpty()) {
			Log4j2.getAuditlog().info(id+" "+endPoint.getEndPointId()+" " +failedMsg.getNotValidData(),endPoint.getEndPointId());
			throw new SqlQueryException(StatusCode.ENDPOINTACESSUPDATEDFAILED.getCode(), failedMsg.getUserUpdationFailed(),
					exception);
		}
		try {
			...
			Log4j2.getAuditlog().info(id+" "+endPoint.getEndPointId()+" " +successMsg.getAccountUpdated(), endPoint.getEndPointId());
